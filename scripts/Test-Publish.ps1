...
$startMenuShortcutComponent = Get-MsiQueryValue -Query "SELECT ``Component_`` FROM ``Shortcut`` WHERE ``Shortcut``='StartMenuShortcut'"
$startMenuShortcutKeyPath = Get-MsiQueryValue -Query "SELECT ``KeyPath`` FROM ``Component`` WHERE ``Component``='StartMenuShortcutComponent'"
$startMenuShortcutRegistry = Get-MsiQueryValue -Query "SELECT ``Registry`` FROM ``Registry`` WHERE ``Component_``='StartMenuShortcutComponent' AND ``Name``='StartMenuShortcut'"
...
$startMenuTarget = Get-MsiQueryValue -Query "SELECT ``Target`` FROM ``Shortcut`` WHERE ``Shortcut``='StartMenuShortcut'"
$desktopTarget = Get-MsiQueryValue -Query "SELECT ``Target`` FROM ``Shortcut`` WHERE ``Shortcut``='DesktopShortcut'"
...
$startMenuArguments = Get-MsiQueryValue -Query "SELECT ``Arguments`` FROM ``Shortcut`` WHERE ``Shortcut``='StartMenuShortcut'"
$desktopArguments = Get-MsiQueryValue -Query "SELECT ``Arguments`` FROM ``Shortcut`` WHERE ``Shortcut``='DesktopShortcut'"
...
Assert-True -Condition ($startMenuShortcutComponent -ceq "StartMenuShortcutComponent") -Message "Start menu shortcut does not use its dedicated lifecycle component."
Assert-True -Condition (-not [string]::IsNullOrWhiteSpace($startMenuShortcutRegistry)) -Message "Start menu shortcut component does not have a registry key path."
Assert-True -Condition ($startMenuShortcutKeyPath -ceq $startMenuShortcutRegistry) -Message "Start menu shortcut component key path is not its registry marker."
...
Assert-True -Condition ($startMenuTarget -ceq "[APPLICATIONFOLDER]S3Explorer.exe") -Message "Start menu shortcut is not a direct executable shortcut: $startMenuTarget"
Assert-True -Condition ($desktopTarget -ceq "[APPLICATIONFOLDER]S3Explorer.exe") -Message "Desktop shortcut is not a direct executable shortcut: $desktopTarget"
Assert-True -Condition ([string]::IsNullOrWhiteSpace($startMenuIcon)) -Message "Start menu shortcut still depends on the MSI Icon table: $startMenuIcon"
Assert-True -Condition ([string]::IsNullOrWhiteSpace($desktopIcon)) -Message "Desktop shortcut still depends on the MSI Icon table: $desktopIcon"
Assert-True -Condition ([string]::IsNullOrWhiteSpace($startMenuArguments)) -Message "Start menu shortcut unexpectedly contains arguments: $startMenuArguments"
Assert-True -Condition ([string]::IsNullOrWhiteSpace($desktopArguments)) -Message "Desktop shortcut unexpectedly contains arguments: $desktopArguments"
...
$frameworkStartMenuTarget = Get-MsiQueryValue -Query "SELECT ``Target`` FROM ``Shortcut`` WHERE ``Shortcut``='StartMenuShortcut'"
$frameworkDesktopTarget = Get-MsiQueryValue -Query "SELECT ``Target`` FROM ``Shortcut`` WHERE ``Shortcut``='DesktopShortcut'"
$frameworkStartMenuIcon = Get-MsiQueryValue -Query "SELECT ``Icon_`` FROM ``Shortcut`` WHERE ``Shortcut``='StartMenuShortcut'"
$frameworkDesktopIcon = Get-MsiQueryValue -Query "SELECT ``Icon_`` FROM ``Shortcut`` WHERE ``Shortcut``='DesktopShortcut'"
...
Assert-True -Condition ($frameworkStartMenuTarget -ceq "[APPLICATIONFOLDER]S3Explorer.exe") -Message "Framework-dependent MSI start menu shortcut is not a direct executable shortcut: $frameworkStartMenuTarget"
Assert-True -Condition ($frameworkDesktopTarget -ceq "[APPLICATIONFOLDER]S3Explorer.exe") -Message "Framework-dependent MSI desktop shortcut is not a direct executable shortcut: $frameworkDesktopTarget"
Assert-True -Condition ([string]::IsNullOrWhiteSpace($frameworkStartMenuIcon)) -Message "Framework-dependent MSI start menu shortcut still depends on the MSI Icon table."
Assert-True -Condition ([string]::IsNullOrWhiteSpace($frameworkDesktopIcon)) -Message "Framework-dependent MSI desktop shortcut still depends on the MSI Icon table."
