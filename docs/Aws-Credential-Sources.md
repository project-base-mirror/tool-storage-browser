...
S3 Explorer 为每个 Amazon S3 连接保存明确的凭据来源。桌面端、CLI、连接测试、Bucket/Object 操作和文件夹同步共用同一个解析器；S3-compatible 连接仍只允许明确保存的密钥，不读取本机 AWS 身份。
...
| AssumeRole | 源 Profile、Role ARN、会话名称、可选 Source Identity、会话时长；DPAPI 加密的 External ID | AWS SDK 使用源 Profile 获取短期角色会话并自动刷新 |
...
- SSO 浏览器 token 位于 AWS SDK 自己的缓存，不进入 `profiles.json`、`.s3connections` 或 DPAPI 长期密钥字段。
...
配置 AssumeRole：
    ...
    s3explorer-cli profile add --name aws-audit-role --type amazon `
        ...
        --external-id-env AWS_AUDIT_EXTERNAL_ID --session-duration 1800
...
- `profiles.json` schema 4 保存连接分组和高级身份配置；Secret Key、Session Token 与 External ID 使用 Windows DPAPI CurrentUser 保护。
- `.s3connections` 格式 3 保留 SSO/AssumeRole/Web Identity 的非敏感引用；账户分组是目标设备的本地组织方式，不随连接包导出。
