...
“凭据 → 权限检查”首先显示权限矩阵：每项凭据一行，列举、读取、上传、删除、ACL、CDN 查询/认证和 CDN 刷新/预热分别以 `√`、`×`、`?`、`—` 表示通过、明确拒绝、无法确定和未记录/不适用。一个凭据关联多个 Bucket、Prefix 或 CDN 域名时，矩阵按最严格结果汇总；阿里云域名查询和通用 HTTP 认证会汇入同一 CDN 只读权限列，具体目标结果可从二级“检查记录”窗口查看。

矩阵中的“立即检查”执行无副作用检查并更新最近结果；“探针检查”只列出当前凭据关联的对象存储连接。需要验证 Put/Delete/ACL 时，每次都必须选择具体连接、Bucket 和非空隔离 Prefix，再输入 `PROBE` 并勾选一次性确认。程序会上传临时对象、按需设置 Private ACL，然后删除临时对象；确认不会保存为永久设置。删除失败会明确提示对象可能残留。
