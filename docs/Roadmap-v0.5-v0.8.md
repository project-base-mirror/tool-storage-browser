...
交付结果：核心模型以 `CredentialSourceKind` 明确区分来源；GUI、CLI、连接测试与所有 S3 操作共用解析器；profile schema 3 和连接包格式 2 只保存外部来源及非敏感 Profile 名称。SSO、AssumeRole 和 Web Identity 已调整到 v0.6.10，解析到这些令牌类型时会明确拒绝而不是静默缓存。
